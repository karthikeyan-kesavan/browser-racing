...
Drive into a labeled event ring, then press its on-screen entry button to start with any car. Four drift challenges score speed, angle, and sustained slides: **Canyon Drift Zone**, **Dockyard Slide**, **Summit Switchbacks**, and **Midnight Burnout**. Each has its own time limit and three-star targets.

Six checkpoint races run against three opponents: **Ridge Rally Rush**, **Browser Racing Sprint**, **Metro Circuit**, **Dustline Dash**, **Coastal Velocity**, and the longer **County Endurance**. Grass events are rally races with rally opponents, while paved events use sport and hypercar opponents. The player's selected car is never restricted. Each race starts with a **3-2-1-GO** countdown that holds every car on the grid before the timer begins. The HUD shows live position, checkpoints, and race time.
